...
class DecidirPagoLogger {
		...
		$this->config = array(
			"appenders" => array(
				"PaymentAppender" => array(
					...
					"params" => array(
						"file" => dirname(__FILE__)."./decidir.log",
					),
					"filters" => array(
						array(
	...
	public function getLogger($payment) {
		DecLogger::configure($this->config);
		...
		if($payment)
			return DecLogger::getLogger("todopagopayment");
		return DecLogger::getLogger("todopagoadmin");
	}
